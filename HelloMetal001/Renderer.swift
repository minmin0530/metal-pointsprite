...
class Renderer: NSObject, MTKViewDelegate {
    ...
    var cube: Cube = Cube(r:1.0,g:0.0,b:0.0,a:1.0,sx:3.0,sy:3.0,sz:3.0)
    var cube2: Cube = Cube(r:0.0,g:1.0,b:0.0,a:1.0,sx:3.0,sy:3.0,sz:3.0)
    var cube3: Cube = Cube(r:0.0,g:0.0,b:1.0,a:1.0,sx:3.0,sy:3.0,sz:3.0)

    var vertexBuffer :MTLBuffer?
    var vertexBuffer2 :MTLBuffer?
    var vertexBuffer3 :MTLBuffer?
    ...
    private func updateGameState() {
        ...
        let viewMatrix = matrix_lookAt(eye: SIMD3<Float>(7,7,7), target: SIMD3<Float>(0,0,0), up:SIMD3<Float>(0,1,0))//matrix4x4_translation(8.0, 8.0, -8.0)
        uniforms[0].modelViewMatrix = simd_mul(viewMatrix, modelMatrix)
        uniforms[0].lightPosition = SIMD3<Float>(1,1,1)
        rotation += 0.003
    }
...
//                renderEncoder.setFrontFacing(.counterClockwise)
                ...
                vertexBuffer2 = device.makeBuffer(bytes: cube2.vertexData, length: 82 * cube.vertexData.count, options:[])!
                vertexBuffer3 = device.makeBuffer(bytes: cube3.vertexData, length: 82 * cube.vertexData.count, options:[])!

                renderEncoder.setVertexBuffer(vertexBuffer!, offset: 0, index: 0)
                renderEncoder.drawPrimitives(type: .point, vertexStart: 0, vertexCount: 36, instanceCount: 36)
